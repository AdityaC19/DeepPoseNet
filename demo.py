...
def main(model_folder,
    ...
    # Print the list of joint coordinates
    print("\nList of Joint Coordinates:\n", joints)

    # Plot the joints in a 3D space using matplotlib
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')

    # Extract x, y, z coordinates from joints
    x = joints[:, 0]
    y = joints[:, 1]
    z = joints[:, 2]

    # Plot the joints as red dots
    ax.scatter(x, y, z, c='r', marker='o', s=30, label='Joints')

    # Set plot limits to create a cube space
    max_range = max(x.ptp(), y.ptp(), z.ptp()) / 2.0
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title('3D Plot of SMPL Joints')
